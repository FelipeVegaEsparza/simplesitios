...
.media-selector-modal.active,
.media-selector-overlay.active {
    display: flex;
}
</style>
@endpush
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-bold" style="color: var(--text-primary);">Nueva Sección</h1>
            <p class="text-sm" style="color: var(--text-secondary);">Crea una nueva sección de contenido</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.sections.store') }}" method="POST" class="space-y-6">
        ...
        <div class="app-card rounded-xl p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Type -->
                <div>
                    <label for="type" class="block text-sm font-medium mb-2" style="color: var(--text-secondary);">
                        Tipo <span class="text-red-500">*</span>
                    </label>
                    <select name="type" id="type" required
                            class="app-input block w-full px-4 py-3 rounded-lg text-sm focus:outline-none">
                        <option value="collection" {{ old('type') === 'collection' ? 'selected' : '' }}>Colección (múltiples entradas)</option>
                        <option value="single" {{ old('type') === 'single' ? 'selected' : '' }}>Único (una sola entrada)</option>
                    </select>
                    @error('type')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3"
                              ...
                              placeholder="Descripción de la sección...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <!-- Image Selector -->
                <div class="md:col-span-2" x-data="{ showMediaSelector: false, selectedImage: null }">
                    ...
                    <input type="hidden" name="image_id" x-ref="imageId" value="{{ old('image_id') }}">
                    @error('image_id')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="endpoint_slug" id="endpoint_slug" value="{{ old('endpoint_slug') }}"
                           class="app-input block w-full px-4 py-3 rounded-lg text-sm focus:outline-none"
                ...
                <div class="md:col-span-2 pt-4" style="border-top: 1px solid var(--border-color);" 
                     x-data="{ showGallerySelector: false, galleryImages: [] }">
                ...
                <div class="md:col-span-2 flex flex-wrap gap-6 pt-4" style="border-top: 1px solid var(--border-color);">
                    ...
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_public_endpoint" value="1" {{ old('is_public_endpoint') ? 'checked' : '' }}
                               class="w-4 h-4 rounded border-slate-300 text-primary-600 focus:ring-primary-500">
