                        ...
                        <tr class="transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.content_entries.destroy', ['client' => $entry->client, 'section' => $entry->section, 'entry' => $entry]) }}" class="inline" onsubmit="return confirm('¿Eliminar esta entrada?')">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    <form method="POST" action="{{ route('admin.content_entries.destroy', ['client' => $entry->client_id ?? 0, 'section' => $entry->section_id ?? 0, 'entry' => $entry]) }}" class="inline" onsubmit="return confirm('¿Eliminar esta entrada huérfana?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-2 rounded-lg transition-colors text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30" title="Eliminar entrada huérfana">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                            </svg>
                                        </button>
