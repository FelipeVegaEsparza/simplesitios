...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h1 class="text-2xl font-bold" style="color: var(--text-primary);">Nueva Entrada</h1>
            <p class="text-sm" style="color: var(--text-secondary);">{{ $section->name }}</p>
        </div>
    </div>

    <form action="{{ route('admin.content_entries.store', ['client' => $client, 'section' => $section]) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    @foreach($section->fields as $field)
                        <div class="{{ $field->column_width === 'full' ? 'md:col-span-2' : '' }}">
                            ...
                            @switch($field->type)
                                @case('text')
                                    <input type="text" name="fields[{{ $field->slug }}]" id="field_{{ $field->slug }}" value="{{ old('fields.' . $field->slug) }}" 
                                ...
                                @case('textarea')
                                    <textarea name="fields[{{ $field->slug }}]" id="field_{{ $field->slug }}" rows="4" 
                                              @if($field->is_required) required @endif 
                                              class="app-input block w-full px-4 py-3 rounded-lg text-sm focus:outline-none resize-none">{{ old('fields.' . $field->slug) }}</textarea>
                                    @break
                                @case('number')
                                    <input type="number" name="fields[{{ $field->slug }}]" id="field_{{ $field->slug }}" value="{{ old('fields.' . $field->slug) }}" 
                                           @if($field->is_required) required @endif 
                                           class="app-input block w-full px-4 py-3 rounded-lg text-sm focus:outline-none">
                                    @break
                                @case('boolean')
                                    <label class="flex items-center gap-3 cursor-pointer">
                                        <input type="checkbox" name="fields[{{ $field->slug }}]" id="field_{{ $field->slug }}" value="1" {{ old('fields.' . $field->slug) ? 'checked' : '' }}
