                        ...
                        <tr class="transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    @if($user->id !== auth()->id())
                                        <form action="{{ route('admin.users.destroy', $user) }}" method="POST" class="inline" id="delete-form-user-{{ $user->id }}" data-modal="global-delete-modal">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" onclick="openDeleteModal('delete-form-user-{{ $user->id }}')" class="p-2 rounded-lg transition-colors text-red-600" onmouseover="this.style.backgroundColor='var(--bg-tertiary)'" onmouseout="this.style.backgroundColor='transparent'" title="Eliminar">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
