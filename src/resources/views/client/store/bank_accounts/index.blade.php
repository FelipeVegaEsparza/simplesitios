            ...
            <div class="app-card rounded-xl p-6 {{ $account->is_default ? 'ring-2 ring-primary-500' : '' }}">
                <div class="flex items-start justify-between">
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        <form method="POST" action="{{ route('client.store.bank-accounts.destroy', $account) }}" class="inline" id="delete-form-bank-{{ $account->id }}" data-modal="global-delete-modal">
                            @csrf
                            @method('DELETE')
                            <button type="button" onclick="openDeleteModal('delete-form-bank-{{ $account->id }}')" class="p-2 rounded-lg transition-colors text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
